...
Key terms used across this portfolio.
...
| Gateway | API proxy layer that applies policies (auth, rate limiting, caching, etc.) to requests |
| Virtual Key | A gateway-issued API key with scoped quotas and permissions |
| Namespace | A logical grouping of tools or resources with access policies |
| ADR | Architecture Decision Record — documented record of a design choice |
| OKR | Objectives and Key Results — goal-setting framework |
| PRD | Product Requirements Document |
| NPV | Net Present Value — present value of future cash flows |
| T2FR | Time to First Report — key activation metric |
